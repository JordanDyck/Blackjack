...
const winner = () => {
  if (isWinner === true)
    if (
      parseInt(pTotal.textContent) > parseInt(cTotal.textContent) &&
      !(parseInt(pTotal.textContent) > 21)
    ) {
      ...
      parseInt(cTotal.textContent) >= parseInt(pTotal.textContent) &&
      !(parseInt(cTotal.textContent) > 21)
    ) {
...
const reShuffle = () => {
  fetch("https://deckofcardsapi.com/api/deck/xtk8kamw3cw9/return/").then(
    (res) => res.json()
  )
}
